...
> **Nota**: Substitua `<URL_DO_SEU_REPOSITORIO>` pela URL do seu repositório (ex.: `https://github.com/renato/cvat`).
...
1. No arquivo `docker-compose.yml`, os blocos de labels para `cvat_server` e `cvat_ui` já estão configurados:
...
2. No arquivo `docker-compose.custom.override.yml`, o serviço `traefik` expõe as portas:
...
3. A sub-rede está configurada no `docker-compose.custom.override.yml` para evitar conflitos de IPs:
...
4. Reinicie os serviços para aplicar as configurações:
...
Abaixo está a lista de containers do stack `cvat` com seus respectivos IPs fixos na rede `cvat_cvat` (`172.20.0.0/24`), conforme configurado nos arquivos `docker-compose.yml`, `docker-compose.serverless.yml`, e `docker-compose.custom.override.yml`:
...
2. O arquivo `docker-compose.custom.override.yml` configura o volume compartilhado para os seguintes serviços:
...
3. Suba o CVAT com os arquivos de configuração:
...
```bash
docker exec -it cvat_worker_annotation ls /home/django/share
```
...
O CVAT suporta integração com cloud storage para gerenciar imagens e anotações. Configuramos o MinIO como provedor S3 local no `docker-compose.custom.override.yml`.

### 🛠️ Etapas de configuração

1. O serviço `minio` está configurado no `docker-compose.custom.override.yml`:
...
3. Instale o MinIO Client (`mc`) para gerenciar o bucket:
...
4. Acesse o CVAT e configure o cloud storage:
...
5. Configure a política do bucket usando o `mc`:
...
6. Faça upload de imagens para teste:
...
7. Crie uma tarefa usando o cloud storage:
...
1. Certifique-se de que os arquivos `function-gpu.yaml`, `main.py`, `model_handler.py` estão na pasta `sam2/nuclio`, e o script `deploy_gpu.sh` está no diretório raiz do repositório:
...
```
  ...
  - Confirme que o `docker-compose.custom.override.yml` usa a sub-rede `172.20.0.0/24` com IPs fixos:
    - `nuclio`: `172.20.0.5`
    - `cvat_server`: `172.20.0.30`
    - `minio`: `172.20.0.20`
    - Outros serviços: `172.20.0.2` a `172.20.0.19`, `172.20.0.21`, `172.20.0.22`
...
- [x] Inclusão dos arquivos no repositório:
  - `function-gpu.yaml`
  - `main.py`
  - `model_handler.py`
  - `deploy_gpu.sh`
...
- [x] Configuração do serviço `minio` com IP fixo `172.20.0.20` no `docker-compose.custom.override.yml`
